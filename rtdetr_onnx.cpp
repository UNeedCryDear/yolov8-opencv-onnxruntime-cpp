...
bool RTDETROnnx::ReadModel(const std::string &modelPath, bool isCuda,
...
int RTDETROnnx::PreProcessing(const std::vector<cv::Mat> &srcImgs,
...
bool RTDETROnnx::OnnxDetect(cv::Mat &srcImg,
...
bool RTDETROnnx::OnnxBatchDetect(
  ...
  for (int img_index = 0; img_index < srcImgs.size(); ++img_index) {
    cv::Mat output0 = cv::Mat(
        cv::Size((int)_outputTensorShape[2], (int)_outputTensorShape[1]),
        CV_32F, all_data);
    ...
    for (int r = 0; r < rows; ++r) { // stride
      ...
      if (max_class_socre >= _classThreshold) {
        ...
        // rect [x,y,w,h]
        float x = (pdata[0] * input_tensor_width - params[img_index][2]) /
                  params[img_index][0]; // x
        float y = (pdata[1] * input_tensor_height - params[img_index][3]) /
                  params[img_index][1];                                  // y
        ...
        cv::Rect box = cv::Rect(left, top, int(w + 0.5), int(h + 0.5));
