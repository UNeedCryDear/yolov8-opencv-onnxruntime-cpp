...
bool Yolov8SegOnnx::ReadModel(const std::string &modelPath, bool isCuda,
  ...
  try {
    ...
    if (_outputNodesNum != 2) {
      std::cout << "This model has " << _outputNodesNum
                << "output, which is not a segmentation model.Please check "
...
bool Yolov8SegOnnx::OnnxBatchDetect(
  ...
  std::vector<int> mask_protos_shape = {1, (int)_outputMaskTensorShape[1],
  ...
  for (int img_index = 0; img_index < srcImgs.size(); ++img_index) {
    ...
    cv::Mat mask_protos =
        cv::Mat(mask_protos_shape, CV_32F,
                output_tensors[1].GetTensorMutableData<float>() +
                    img_index * mask_protos_length);
    for (int i = 0; i < temp_mask_proposals.size(); ++i) {
      GetMask2(cv::Mat(temp_mask_proposals[i]).t(), mask_protos, temp_output[i],
               mask_params);
    }
